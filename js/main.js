  ...
  $('#search-button').click(function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        response._embedded.events.forEach(function (character) {


          var data = {
            name: character.name,
            img: character.images[0].url,
            url: character.url,
          };
...
function getText(event) {
  ...
  var valueTextArea = $(".post-text").val();
  console.log(valueTextArea);
  addComment(valueTextArea);
...
var template = '<div class="row">' +
  ...
  '<div class="col">' +
  // '<button type="button" class="publish">Post</button>' +
  '<p>_comment_<p>'
'</div>' +
'</div>' +
'</div>'
...
function addComment(valueTextArea) {
  var finalTemplate = '';
  finalTemplate = template.replace('_comment_', valueTextArea);
  $(".post").append(finalTemplate);
}